...
func (i *sdkInjector) inject(ctx context.Context, insts languageInstrumentations, ns corev1.Namespace, pod corev1.Pod, containerName string) corev1.Pod {
	...
	if insts.Apache != nil {
		otelinst := *insts.Apache
		i.logger.V(1).Info("injecting apache instrumentation into pod", "otelinst-namespace", otelinst.Namespace, "otelinst-name", otelinst.Name)
		...
		pod = injectWebserverAgent(otelinst.Spec.Webserver, pod, index, otelinst.Spec.Endpoint, i.createResourceMap(ctx, otelinst, ns, pod, index), apache)
		pod = i.injectCommonEnvVar(otelinst, pod, index)
		pod = i.injectCommonSDKConfig(ctx, otelinst, ns, pod, index)
	}
	if insts.Nginx != nil {
		...
		// Therefore, service name, otlp endpoint and other attributes are passed to the agent injection method
		pod = injectWebserverAgent(otelinst.Spec.Webserver, pod, index, otelinst.Spec.Endpoint, i.createResourceMap(ctx, otelinst, ns, pod, index), nginx)
		pod = i.injectCommonEnvVar(otelinst, pod, index)
		pod = i.injectCommonSDKConfig(ctx, otelinst, ns, pod, index)
	}
